...
# import utils.loss_functions.setcriterion as box_ops
...
class SetCriterion(nn.Module):
    ...
    def get_labels(self, outputs, targets, indices, num_boxes, log=True):
        assert 'pred_boxes' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_boxes = outputs['pred_boxes'][idx]
        target_boxes = torch.cat([t['boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0)
        return src_boxes, target_boxes
    def get_cls(self, outputs, targets, indices, num_boxes):
        assert 'pred_logits' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_logits = outputs['pred_logits'][idx]     # prediction
        target_classes_o = torch.cat([t["labels"][J] for t, (_, J) in zip(targets, indices)])
        ...
        scores, labels = src_logits.sigmoid().max(-1)
        return (scores, labels), target_classes_o
    ...
    def get_boxes(self, outputs, targets, indices, num_boxes):
        assert 'pred_boxes' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_boxes = outputs['pred_boxes'][idx]
    ...
    def loss_boxes_ciou(self, outputs, targets, indices, num_boxes):
        """Compute the losses related to the bounding boxes, the L1 regression loss and the GIoU loss
           targets dicts must contain the key "boxes" containing a tensor of dim [nb_target_boxes, 4]
           The target boxes are expected in format (center_x, center_y, w, h), normalized by the image size.
        """
        ...
        loss_giou = 1 - torch.diag(box_ops.complet_box_iou(
            box_ops.box_cxcywh_to_xyxy(src_boxes),
            box_ops.box_cxcywh_to_xyxy(target_boxes)))
        losses['loss_ciou'] = loss_giou.sum() / num_boxes
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            'boxes_ciou' : self.loss_boxes_ciou
        }
