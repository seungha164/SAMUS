...
def evaluate(test_loader, model, criterion, opt, args):
    ...
    device = opt.device
    N = len(test_loader)
    losses_result_dict = {'ce' : 0., 'bbox': 0., 'ciou': 0., 'total': 0.}
    ...
    for idx, (imgs, prompts, targets) in enumerate(tqdm(test_loader)):
        ...
        losses_result_dict['ciou'] += loss_dict['loss_ciou'].item()
    ...
    return {
        ...
        'loss_ciou'     : (losses_result_dict['ciou'] / N),
...
def main():
    ...
    weight_dict = {"loss_ce": 1, "loss_bbox": l1_weight, "loss_ciou": giou_weight}
    if deep_supervision:
        aux_weight_dict = {}
        for i in range(dec_layers - 1):
            aux_weight_dict.update({k + f"_{i}": v for k, v in weight_dict.items()})
        weight_dict.update(aux_weight_dict)
    losses = ["labels", "boxes_ciou", "cardinality"]
    ...
    for epoch in range(opt.epochs):
        ...
        if epoch % opt.eval_freq == 0:
            ...
            print('epoch [{}/{}], val loss-ce:{:.4f}, loss-bbox:{:.4f}, loss-ciou:{:.4f}'.format(
                epoch, opt.epochs, val_results['loss_ce'], val_results['loss_bbox'], val_results['loss_ciou']))
            ...
            if args.keep_log:
                ...
                TensorWriter.add_scalar('mean_loss_ciou', val_results['loss_ciou'], epoch)
